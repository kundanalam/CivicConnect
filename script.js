...
document.addEventListener("DOMContentLoaded", function(){
    ...
    const response = await fetch(
        "http://127.0.0.1:5000/submit-complaint",
        {
            method: "POST",
            body: formData
        }
...
async function loadAreas() {
    ...
    try {
        ...
        const res = await fetch("http://127.0.0.1:5000/area-complaints");
...
if(village){
    ...
    url = "http://127.0.0.1:5000/complaints-by-village/" + village;

}else{

    url = "http://127.0.0.1:5000/all-complaints";
...
async function markResolved(complaintId){
    ...
    const res = await fetch(
        "http://127.0.0.1:5000/update-status/" + complaintId,
        {
            method: "PUT"
        }
    );
...
function downloadExcel(){
    window.open("http://127.0.0.1:5000/download-excel");
}
async function trackComplaint(){

    const id = document.getElementById("trackId").value;

    const res = await fetch(`http://127.0.0.1:5000/track-complaint/${id}`);
    const data = await res.json();
...
async function trackComplaint(){
    ...
    `;
>>>>>>> 71b3e4e4f6f902c95fc9786afb96cfff1a161ab1
}